const {
  createReportUC,
} = require("../../application/services/report/uploadAssetsToDB.uc");
const {
  getAllReportsUC,
} = require("../../application/services/report/getAllreports.uc");
const {
  reportExistenceCheckUC,
} = require("../../application/services/report/reportExistenceCheck.uc");
const {
  addCommonFields,
} = require("../../application/services/report/addCommonFields.uc");
const {
  checkMissingPagesUC,
} = require("../../application/services/report/checkMissingPages.uc");
const {
  getReportsByUserIdUC,
} = require("../../application/services/report/getReportsByUserId.uc");
...
const reportController = {
  ...
  async reportExistenceCheck(req, res) {
    try {
      ...
      const companyOfficeId = extractCompanyOfficeId(req);
      const { success, message, data } = await reportExistenceCheckUC(
        reportId,
        companyOfficeId,
      );
  ...
  async getAllReports(req, res) {
    try {
      ...
      console.log(
        `[getAllReports] Successfully fetched ${result.data.length} reports out of ${result.pagination.totalItems} total`,
      );
  ...
  async getReportsByUserId(req, res) {
    try {
      ...
      const { success, message, data, pagination } = await getReportsByUserIdUC(
        {
          userId,
          page: req.query.page,
  ...
  async checkMissingPages(req, res) {
    try {
      const { reportId } = req.params;
      const companyOfficeId = extractCompanyOfficeId(req);
      const { success, missingPages, hasMissing } = await checkMissingPagesUC(
        reportId,
        companyOfficeId,
      );
  ...
  async addCommonFields(req, res) {
    try {
      ...
      const companyOfficeId = extractCompanyOfficeId(req);
      const { success, message, data } = await addCommonFields(
        reportId,
        region,
        city,
        inspectionDate,
  ...
  async createReportWithCommonFields(req, res) {
    try {
      ...
      const enhancedReportData = rawRows.map((entry) => {
        ...
        enhancedEntry.production_capacity =
          enhancedEntry.production_capacity || "0";
        enhancedEntry.production_capacity_measuring_unit =
          enhancedEntry.production_capacity_measuring_unit || "0";
        enhancedEntry.product_type = enhancedEntry.product_type || "0";
        enhancedEntry.country =
          enhancedEntry.country || "المملكة العربية السعودية";
      ...
      const normalizedStatus = (reportStatus || (storeOnlyFlag ? "DRAFT" : ""))
        .toString()
        .trim();
      ...
      try {
        ...
        console.warn(
          "Failed to create report failure notification",
          notifyError,
        );
      }
      return res.status(500).json({
  ...
  async updateCompanyOfficeId(req, res) {
    try {
      ...
      const companyOfficeId = extractCompanyOfficeId(req);
      const userIdRaw =
        req.userId || req.user?.id || req.user?._id || req.body?.user_id;
      ...
      if (!companyOfficeId) {
        return res
          .status(400)
          .json({ success: false, message: "companyOfficeId is required" });
      }
      if (!userIdRaw) {
        return res
          .status(401)
