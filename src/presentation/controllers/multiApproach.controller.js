...
// Instead of throwing error for unmatched PDFs, you can still log or ignore
if (unmatchedPdfs.length > 0) {
  console.warn(
    "These PDFs do not match any Excel file by name: " + unmatchedPdfs.join(", ")
  );
}

// Now ensure every Excel has at least one PDF (attach dummy if missing)
// const dummyPdfPath = path.resolve("uploads/static/dummy_placeholder.pdf");
excelMap.forEach((value, baseName) => {
  if (value.pdfs.length === 0) {
    ...
    for (const [baseName, { file, pdfs }] of excelMap.entries()) {
      ...
      // If you want exactly one PDF per Excel:
      if (pdfs.length !== 1) {
        throw new Error(
